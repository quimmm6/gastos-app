...
export default function Stats({ transactions }) {
  ...
  transactions.forEach(tx => {
    ...
    if (!byMonth[m]) byMonth[m] = { mes: monthLabel(tx.fecha), gastos: 0, ingressos: 0 }
    byMonth[m][tx.tipo === 'gasto' ? 'gastos' : 'ingressos'] += tx.importe
  })
  ...
  return (
    <div>
      <div className="stats-section">
        <div className="stats-title">Evolució mensual (últims 6 mesos)</div>
          ...
          <ResponsiveContainer>
            <BarChart data={monthData} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
              ...
              <Tooltip
                ...
                formatter={(v, name) => [fmt(v), name === 'gastos' ? 'Despeses' : 'Ingressos']}
              />
      ...
      <div className="card">
        <div className="stats-title" style={{ marginBottom: 10 }}>Resum total</div>
        {[
          { label: 'Total transaccions', value: transactions.length },
          { label: 'Total despeses', value: fmt(gastos.reduce((s, t) => s + t.importe, 0)) },
          { label: 'Total ingressos', value: fmt(ingresos.reduce((s, t) => s + t.importe, 0)) },
          { label: 'Mitjana despesa', value: gastos.length ? fmt(gastos.reduce((s, t) => s + t.importe, 0) / gastos.length) : '—' },
        ].map(row => (
